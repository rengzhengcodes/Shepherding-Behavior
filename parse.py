"""
Parses the results JSONs produced by the summarizers of every experiment run. Uses
repetition number to isolate the 0th, 25th, 50th, 75th, and 100th percentile of final ticks
by their rng seed. Then, plots the final tick distribution for all modes in a histogram and
boxplot.

Different from functions in save_data as we do not save every state for every tick.
"""
...
for MODE in [0, 2, 3, 4]:
    ...
    files: list = reversed(
        sorted(file for file in os.listdir(folder) if file.endswith(".txt"))
    )
    ...
    isolated_reps = [
        reps_sorted[int(len(reps_sorted) * percentile / 100)]
...
bins = np.linspace(
    np.array(list(mode_runs.values())).min(), results[0]["ITERATIONS"], 100
)
for i, (mode, final_ticks) in enumerate(mode_runs.items()):
    ...
    plt.axvline(
        ...
        linestyle="dashed",
        linewidth=1,
        label=f"{ALIAS[mode]} Mean",
        c=c,
    )
    plt.axvline(
        np.median(final_ticks),
        color="r",
        linestyle="dotted",
        linewidth=1,
        label=f"{ALIAS[mode]} Median",
        c=c,
    )
...
for i, (mode, final_ticks) in enumerate(mode_runs.items()):
    c = plt.cm.tab10(i)
    plt.boxplot(
        final_ticks, positions=[i], patch_artist=True, boxprops={"facecolor": c}
    )
    plt.scatter([i] * len(final_ticks), final_ticks, alpha=0.5, c=c)
plt.xticks(mode_runs, [ALIAS[mode] for mode in mode_runs])
plt.xlabel("Mode")
