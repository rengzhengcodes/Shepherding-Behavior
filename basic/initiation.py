...
def initiate(N_sheep, N_shepherd, space_x, space_y, Target_size):
    ...
    swarm[:, 0] = (
        np.random.uniform(-space_x, space_x, N_sheep)
        if MORPHOLOGY
        else np.random.uniform(50, space_x, N_sheep)
    )
    swarm[:, 1] = (
        np.random.uniform(-space_y, space_y, N_sheep)
        if MORPHOLOGY
        else np.random.uniform(50, space_y, N_sheep)
    )
    ...
    swarm[:, 22] = (
        0.0  ### hull membership: hull -> 1+ designating CCW order; non-hull -> 0.
    )
    swarm[:, 23] = (
        0.0  ### Tracks which shepherd sees the agent using little endian binary encoding. 1 if shepherd sees agent, 0 otherwise.
    )
...
def initiate_shepherds(N_shepherd, agent_num, L3):
    ...
    shepherd_swarm[:, 5] = (
        10 * (np.sqrt(agent_num)) * 2 / 3
    )  # [1/2, 100]=50 [3/4, 100]=75 [4/5, 100]=80  #7.5*(np.sqrt(agent_num))*2/3  7.5: repulsion_distance  # L1: drive point: from shepherd to mass center(66.6 Ns = 100)
    shepherd_swarm[:, 6] = 1  # v0    #4 #2 2 is too small to catch up the sheep
    shepherd_swarm[:, 7] = (
        1  # 1               # alpha # acceleration rate #4 #3 #1 #2  ##3.5
    )
    shepherd_swarm[:, 8] = (
        0.1  # 0.1             # beta  # turning rate #2 #0.5 #1        ##0.5
    )
    ...
    shepherd_swarm[:, 12] = (
        10 * (np.sqrt(agent_num)) * 2 / 3
    )  # *2/3 #*3/4 #7.5*(np.sqrt(agent_num))*2/3 # 7.5 = 2.5*3 # L2: d_furthest in collect mode:(35 Ns = 50)(50 Ns=100)(70.7 Ns = 200)
    ...
    shepherd_swarm[:, 18] = (
        np.pi / 3
    )  # Relative FOV threshold for drive mode;   # K_attraction_target   0.01
    shepherd_swarm[:, 19] = (
        L3  # L3: distance to avoid other shepherd #10 ((np.sqrt(agent_num))*7.5)/3/(N_shepherd+1)
    )
