...
def initiate(N_sheep, N_shepherd, space_x, space_y, Target_size):
    ...
    swarm[:, 5] = 0     # attraction_distance #25  #30
...
def initiate_shepherd(N_shepherd, agent_num, L3):
    ...
    shepherd_swarm[:, 5] = 10*(np.sqrt(agent_num))*2/3 #[1/2, 100]=50 [3/4, 100]=75 [4/5, 100]=80  #7.5*(np.sqrt(agent_num))*2/3  7.5: repulsion_distance  # L1: drive point: from shepherd to mass center(66.6 Ns = 100)
    ...
    shepherd_swarm[:, 12] = 10*(np.sqrt(agent_num))*2/3 #*2/3 #*3/4 #7.5*(np.sqrt(agent_num))*2/3 # 7.5 = 2.5*3 # L2: d_furthest in collect mode:(35 Ns = 50)(50 Ns=100)(70.7 Ns = 200)
