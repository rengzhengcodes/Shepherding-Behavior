...
def update_agents_state(
    ...
    """
    Update the state of the agents based on their distance to the target.
    Args:
        agents: The agents.
...
def update(agents, shepherd, target_x, target_y):
    ...
    _, f_shepherd_force_x, f_shepherd_force_y = get_shepherd_force(
        agents, shepherd)
    ...
    for agent_index in range(agents.shape[0]):
        ...
        w_dot = (
            -f_x * np.sin(agents[agent_index][2]) + f_y *
            np.cos(agents[agent_index][2])
        ) * (
            1 / v0
        )  # inertia
...
@nb.jit(nopython=True)
def get_furthest_agent(agents, shepherd_x, shepherd_y,
    ...
    for agent_index in range(num_agents):
        ...
        if agents[agent_index][21] == 0:
            ...
            # angle between two vector: [-np.pi, np.pi]
            # negative: the agent its on the left side of the target
...
def drive_the_herd_using_visible_convex_hull(
    agents, shepherd_x, shepherd_y, shepherd_index, target_place_x, target_place_y
):
...
def drive_the_herd_using_subflock_convex_hulls(
    ...
    # Set hull and visibility status.
    agents[visible_hulls_section, 22] = np.arange(
        1, visible_hulls_section.shape[0] + 1)
    ...
    with nb.objmode():
        agents[visible_hulls_section, 23] = (
            agents[visible_hulls_section, 23].view(
                "uint64") | (0b01 << shepherd_index)
        ).view("float64")
...
def herd(agents, shepherd, target_place_x, target_place_y):
    ...
    for shepherd_index in range(shepherd.shape[0]):
        ...
        if shepherd[shepherd_index][13] == 1.0:
            ...
            if MODE == 0:
                # find the drive point and calculate the force attraction from
                # the drive point; drive_point_x,
            ...
            if MODE == 1:
                ...
                if (
                    np.absolute(
                        max_angle_target_to_agent) > Angle_Threshold_Collection
            ...
            if MODE == 1:
                ...
                angle_difference_agent_mass = collect_the_herd_using_vision(
                    collect_agent_id, agents, shepherd_x, shepherd_y
                )
                ...
                # if the agent is closer enough to the center or the agents are
        ...
        shepherd[shepherd_index][0] = (
            shepherd_x + ((v0 + v_dot * alpha) *
                          np.cos(shepherd_angle)) * tick_time
        )
        shepherd[shepherd_index][1] = (
            shepherd_y + ((v0 + v_dot * alpha) *
                          np.sin(shepherd_angle)) * tick_time
        )
...
@nb.jit(nopython=True)
def evolve(agents, shepherd, Target_place_x,
    ...
    agents_update = update_agents_state(
        agents_update, Target_place_x, Target_place_y, Target_size
    )
